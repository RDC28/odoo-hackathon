...
from models import client, db, ensure_indexes, utcnow
...
from models.conversation import new_conversation, new_message
...
def seed():
    ...
    def make_vehicle(company, owner, vehicle_type, model, registration, seats, mileage):
        ...
        rating_avg = round(4.4 + (len(owner['name']) % 6) * 0.1, 1)
        rating_count = 6 + (len(owner['name']) % 15)
        db.users.update_one(
            {'_id': owner['_id']},
            {'$set': {'rating_avg': rating_avg, 'rating_count': rating_count}},
        )
        owner['rating_avg'] = rating_avg
        owner['rating_count'] = rating_count
        return vehicle
    ...
    def make_ride(company, driver, vehicle, start, destination, hour, seats, price,
                  add_days=0, recurring=None, status="active"):
        ride = new_ride(
    ...
    def set_ride_seats(ride, seats_available):
        ride['seats_available'] = seats_available
        db.rides.update_one({'_id': ride['_id']}, {'$set': {'seats_available': seats_available}})

    def make_booking(ride, rider, seats=1, status="booked"):
        conversation = new_conversation(
            company_id=ride['company_id'], type="ride",
            ride_id=ride['_id'], last_message_at=utcnow().isoformat(),
            participant_ids=[rider['_id'], ride['driver_id']],
        )
        ...
        db.rides.update_one({'_id': ride['_id']}, {'$inc': {'seats_available': -seats}})
    ...
    db.messages.insert_one(new_message(
        conversation_id=history_chat['_id'], sender_id=priya['_id'], content="Thanks for the ride!",
    ))
    ...
    for company_index, (company_name, industry, address, join_code, admin_email) in enumerate(generated_companies):
        ...
        for driver_index, (driver, vehicle) in enumerate(zip(drivers, vehicles)):
            ...
            for history_index in range(3):
                ...
                set_ride_seats(history, max(0, history['seats_total'] - 2))
    ...
    db.users.update_one(
        {'_id': suspended['_id']},
        {'$set': {'status': 'suspended', 'rating_avg': 0, 'rating_count': 0}},
    )
    db.vehicles.update_one({'_id': inactive_vehicle['_id']}, {'$set': {'status': 'inactive'}})
